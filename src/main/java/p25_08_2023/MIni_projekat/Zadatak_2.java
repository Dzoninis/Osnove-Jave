...
//|     slika |x Uros Urosevic
        ...
        ArrayList<String> aktivniKorsnici = new ArrayList<>();
        ArrayList<String> neaktivniKorsnici = new ArrayList<>();


        boolean daLiJeAktivan;

        for (int i = 0; i < aktivnosti.size(); i++) {
            daLiJeAktivan = aktivnosti.get(i);
            if (daLiJeAktivan) {
                aktivniKorsnici.add(imena.get(i));
            } else {
                neaktivniKorsnici.add(imena.get(i));
            }
        }

        for (int i = 0; i < aktivniKorsnici.size(); i++) {
            System.out.println("slika |o " + aktivniKorsnici.get(i));
        }

        for (int i = 0; i < neaktivniKorsnici.size(); i++) {
            System.out.println("slika |x " + neaktivniKorsnici.get(i));
        }
    }
}
